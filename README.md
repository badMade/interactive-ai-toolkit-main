...
Use the universal LLM command line tools after installing the dependencies:
...
text from the same entry point:

```bash
# Stream tokens as they arrive (add --json for structured deltas)
...
python -m universal_llm.cli tools "Generate a differentiated lesson plan" \
  --tools path/to/tools.json --provider openai --model gpt-4o-mini
...
`--config path/to/settings.json` (or another supported format) to centralize API
keys and defaults shared by multiple commands.
