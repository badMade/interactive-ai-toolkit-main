...
return dedent(
    ...
    - Help educators and learners prototype inclusive lessons without relying on
    ...
    - Internet access the first time Whisper and SpeechT5 model weights download
      from PyPI and Hugging Face.
    ...
    ------------------------------
    - ``lesson_recording.mp3`` is intentionally excluded so the repository stays
      lightweight—add your own audio when following the walkthrough.
    ...
    4. Copy or record an audio sample (for example ``lesson_recording.mp3``) into
       the project root.
    5. Run offline transcription: ``python transcribe.py [audio_path] [--model MODEL_NAME]``
       and review the printed transcript.
    6. Generate speech from text: ``python tts.py`` to create ``output.wav`` using
       the deterministic SpeechT5 voice.
    ...
    2. Configure ``OPENAI_API_KEY`` for repeatable use.
       - Windows (PowerShell): ``setx OPENAI_API_KEY "sk-..."`` then start a new
         shell; for the current session use ``$env:OPENAI_API_KEY = "sk-..."``.
    ...
    4. Use the hosted option when managed infrastructure or faster turnaround is
       preferable to local hardware.
    ...
    --------------------
    - Whisper ``FileNotFoundError`` or FFmpeg errors: ensure FFmpeg is installed
      and accessible via PATH; verify with ``ffmpeg -version``.
