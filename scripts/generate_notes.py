...
def build_notes() -> str:
    ...
    return dedent(
        ...
        - Help educators and learners prototype inclusive lessons without relying on proprietary cloud services.
        ...
        - Internet access the first time Whisper and SpeechT5 model weights download from PyPI and Hugging Face.
        - Optional: the ``openai`` Python client when you plan to call the hosted Whisper API.
        ...
        4. Install FFmpeg (required for Whisper audio decoding).
           - Windows: download from https://www.gyan.dev/ffmpeg/builds/ and add ``ffmpeg.exe`` to PATH or copy it beside ``transcribe.py``
           - macOS: ``brew install ffmpeg``
           - Linux: ``sudo apt install ffmpeg``
        5. (Optional) Install the OpenAI Python client: ``python -m pip install openai``

        Why Some Files Are Not Tracked
        ------------------------------
        - ``lesson_recording.mp3`` is intentionally excluded so the repository stays lightweight—add your own audio when following the walkthrough.
        - Hugging Face caches for SpeechT5 live under ``~/.cache/huggingface`` and do not belong in source control.

        Internal Architecture Overview
        -------------------------------
        - ``transcribe.py`` wraps Whisper to load audio, perform transcription, and print the resulting text.
        - ``tts.py`` loads SpeechT5 and its vocoder to convert text into ``output.wav`` using a deterministic speaker embedding.
        - ``requirements.txt`` centralises the Python dependencies required by both command-line tools.
        ...
        3. Install FFmpeg (Windows builds from Gyan.dev, ``brew install ffmpeg`` on macOS, or ``sudo apt install ffmpeg`` on Debian/Ubuntu) and confirm the command works with ``ffmpeg -version``.
        ...
        7. Import or adapt the scripts inside other Python programs to automate inclusive lesson preparation.
