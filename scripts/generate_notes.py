...
- ``scripts/`` contains automation helpers to streamline maintenance:
  - ``ensure_requirements.py`` validates dependency consistency.
  - ``generate_notes.py`` rebuilds ``notes.txt``.
  - ``install_ffmpeg.sh`` bootstraps FFmpeg on supported platforms.
  - ``self_debug.py`` runs interactive diagnostics for troubleshooting.
  - ``verify_environment_setup.py`` sanity-checks project prerequisites.
