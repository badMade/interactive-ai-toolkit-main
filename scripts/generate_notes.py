...
------------------------------
- ``lesson_recording.mp3`` stays in standard Git so reproducible demos ship
  with an identical audio sample without depending on Large File Storage.
- Hugging Face caches for SpeechT5 live under ``~/.cache/huggingface`` and
  remain untracked because they are machine specific.
...
- ``requirements.txt`` centralises the Python dependencies required by both
  command-line tools.
- ``scripts/`` contains automation helpers to streamline maintenance:
  - ``ensure_requirements.py`` validates Python dependency consistency
    against the lockfile expectations.
  - ``generate_notes.py`` regenerates ``notes.txt`` from the latest
    transcription and synthesis outputs.
  - ``install_ffmpeg.sh`` bootstraps FFmpeg on supported platforms for audio
    preprocessing.
  - ``self_debug.py`` provides interactive diagnostics for troubleshooting
    local environments.
  - ``verify_environment_setup.py`` performs sanity checks to confirm
    project prerequisites are satisfied.
